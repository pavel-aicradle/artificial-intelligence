...
class ActionLayer(BaseActionLayer):
	...
	def _inconsistent_effects(self, actionA, actionB): # these things are ActionNodes, not Actions.
		...
		layers.ActionNode
		"""
		# really we need to check that the add actions from one are not in the remove actions of the other
		for a in actionA.effects:
			if ~a in actionB.effects: return True

		for b in actionB.effects:
			if ~b in actionA.effects: return True
	...
	def _interference(self, actionA, actionB):
		...
		"""
		for a in actionA.effects:
			if ~a in actionB.preconditions: return True

		for b in actionB.effects:
			if ~b in actionA.preconditions: return True
	...
	def _competing_needs(self, actionA, actionB):
		...
		for a in actionA.preconditions:
			for b in actionB.preconditions:
				if self.parent_layer.is_mutex(a, b): return True # need is_mutex(b, a) too?
...
class LiteralLayer(BaseLiteralLayer):
	...
	def _inconsistent_support(self, literalA, literalB):
		...
		layers.BaseLayer.parent_layer
		"""
		for a in self.parents[literalA]: # a is an ActionNode
			for b in self.parents[literalB]:
				if not self.parent_layer.is_mutex(a, b): return False

		return True

	def _negation(self, literalA, literalB): # literals are of type Expr from utils.
		""" Return True if two literals are negations of each other """
		#return literalA^literalB confusingly only creates a new expression, doesn't evaluate it.
...
class PlanningGraph:
	...
	def h_levelsum(self):
		...
		"""
		subgoals = self.goal.copy() # it's a set
		lsum = 0
		level = 0
		while not self._is_leveled:
			...
			for g in layer:
				if g in subgoals:
					lsum += level
					if len(subgoals) == 1: return lsum
					else: subgoals.remove(g)
			self._extend()
			level += 1
		
		return float('inf') # if no solution, then infinite cost
	...
	def h_maxlevel(self):
		...
		while not self._is_leveled:
			...
			for g in layer:
				if g in subgoals:
					if len(subgoals) == 1: return level
					else: subgoals.remove(g)
			self._extend()
			level += 1
	...
	def h_setlevel(self):
		...
		"""
		def all_goal_conditions_present(layer):
			for g in self.goal:
				if g not in layer: return False
			return True

		def no_mutex_between_goal_conditions(layer):
			for g1, g2 in combinations(self.goal, 2):
				if layer.is_mutex(g1,g2): return False
			return True

		level = 0
		while not self._is_leveled:
			layer = self.literal_layers[-1]
			if all_goal_conditions_present(layer) and no_mutex_between_goal_conditions(layer): return level
			self._extend()
			level += 1
